...
**IMPORTANT:** The ASCII dependency graph is rendered by an external script (`scripts/render_dependency_graph.sh`), NOT generated by the LLM.

### Step-by-Step Rendering Process

**REQUIRED SEQUENCE - Follow these steps in order:**

**Step 1: Write to TodoWrite**
- Write ALL tasks to TodoWrite using the encoded format
- Format: `[W<wave>:<title>][<phase_id>][<agent>][PARALLEL]? <description>`
- Ensure all atomic tasks are included with correct wave assignments

**Step 2: Invoke the Render Script**
- After TodoWrite is populated, invoke the render script via Bash tool
- Command: `~/.claude/scripts/render_dependency_graph.sh`
- The script reads from TodoWrite JSON, parses the encoded format, and outputs the ASCII graph
...
1. Reads the current TodoWrite entries (JSON format)
2. Parses the encoded metadata: `[W<n>:<title>][<phase_id>][<agent>][PARALLEL]?`
3. Builds the dependency structure from wave assignments
4. Outputs a deterministic ASCII dependency graph
...
**Note:** The ASCII dependency graph visualization is rendered by `scripts/render_dependency_graph.sh`, not generated by the LLM. After writing TodoWrite entries, invoke the script via Bash: `~/.claude/scripts/render_dependency_graph.sh`
...
**Note:** The visual wave structure is rendered by `scripts/render_dependency_graph.sh`. After writing TodoWrite entries, invoke the script via Bash: `~/.claude/scripts/render_dependency_graph.sh`
...
**Note:** The ASCII dependency graph should already be displayed from Step 2 (render script invocation).
...
Before generating your recommendation output, you MUST complete the task tree JSON and write TodoWrite entries. The dependency graph visualization is rendered by the external script.
...
- ❌ NEVER generate ASCII dependency graphs (script handles this)
...
- After writing to TodoWrite, invoke: `~/.claude/scripts/render_dependency_graph.sh` to render the ASCII dependency graph
...
21. **Script-Rendered Graphs:** After writing to TodoWrite, invoke `~/.claude/scripts/render_dependency_graph.sh` via Bash to render the ASCII dependency graph - do NOT generate ASCII graphs manually
