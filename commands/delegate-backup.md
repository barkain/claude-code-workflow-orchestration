...
- This is the prompt to use in your Task call
...
Use Task tool with the extracted delegation prompt:

```
Task(
  subagent_type: "general-purpose",
  description: "[Agent name from recommendation]: [brief task summary]",
  prompt: "[Extracted delegation prompt from recommendation]"
)
```
...
Task(
  subagent_type: "general-purpose",
  description: "Phase 1 - [Agent name]: [brief phase summary]",
  prompt: "[Extracted Phase 1 delegation prompt]"
)
```
...
2. **Go back to orchestrator** for Phase 2 guidance:
   - Spawn orchestrator agent with Task tool
   - Provide user task + context from Phase 1
   - Request Phase 2 recommendation

   **Example: Re-invoking Orchestrator for Phase 2**

   After Phase 1 completes, invoke the orchestrator again with this pattern:

   ```
   Task(
     subagent_type: "delegation-orchestrator",
     description: "Delegation Orchestrator: Phase 2 guidance",
     prompt: "**ORIGINAL USER TASK:** $ARGUMENTS
...
- Look for any Task tool call instructions in the recommendation
...
**Important Reminders:**
- You will make TWO Task tool calls:
  1. First to orchestrator (get recommendation)
  2. Second to specialized agent (execute delegation)
- For multi-step: Additional orchestrator calls for each subsequent phase
- Parse the delegation prompt from orchestrator's structured output
- Use the extracted prompt verbatim in your delegation Task call
