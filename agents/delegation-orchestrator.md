...
## ASCII Dependency Graph Format (REFERENCE ONLY)

**NOTE: The main agent renders the ASCII dependency graph using `scripts/render_dependency_graph.sh` based on the JSON task tree and TodoWrite entries. This section is provided as a reference for the expected format only.**

**The orchestrator outputs:**
1. JSON task tree (machine-readable)
2. TodoWrite entries with encoded metadata
3. Phase Breakdown (text list)

**The main agent renders:**
- ASCII dependency graph from the JSON/TodoWrite data

**FORMAT REFERENCE (for expected graph appearance):**
...
**Note:** Generate the visual wave structure directly as part of your output using the ASCII graph format specified in the "MANDATORY: ASCII Dependency Graph Visualization" section.
