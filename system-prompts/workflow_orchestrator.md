...
| TaskList polling loop | ~100 per call × N | **NEVER** — wait for automatic notifications |
...
### Step 0: Check Agent Teams Mode (MANDATORY FIRST STEP)
Check if `TeamCreate` is in your available tools. If yes, teams are enabled — ALL agent spawning in this workflow MUST use TeamCreate + Agent(team_name=...). Do NOT run any Bash commands to check env vars (Bash is blocked for the main agent).
...
**File naming:** `$CLAUDE_SCRATCHPAD_DIR/{sanitized_subject}.md` — lowercase, spaces → underscores.
...
**Decision:** When `TeamCreate` is available: default to `execution_mode: "team"` (used when `team_mode_score` is not calculated, absent, or > -3); use `execution_mode: "subagent"` only when `team_mode_score` <= -3. If TeamCreate fails at runtime, the fallback section handles it automatically. Without teams available: always parallel subagent (>=2 subtasks mandatory).
...
### Stage 1: Execution (Team Mode — PRIMARY)

When `TeamCreate` is in your available tools, this is the PRIMARY execution path. Use TeamCreate + Agent(team_name=...) for all multi-agent work.
...
2. **PROHIBITED:** Simplifying plan, collapsing parallel→sequential, changing agents, reordering/skipping/adding phases
